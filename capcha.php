<?php
	...
	function rgb_merge($lhs, $rhs) {
		$color[ 'r' ] = ( $lhs[ 'r' ] + $rhs[ 'r' ] ) / 2;
		$color[ 'g' ] = ( $lhs[ 'g' ] + $rhs[ 'g' ] ) / 2;
		$color[ 'b' ] = ( $lhs[ 'b' ] + $rhs[ 'b' ] ) / 2;
	...
	for ($i = 0; $i < rand(5,9); $i++) {
		...
		imagestring($image, $f, $x, $y, sb_get_capcha(), $color[$i]); 
	}

	// Center the real captcha text
	$x  = ($CONFIG[ 'width' ] - ( ImageFontWidth($CONFIG[ 'font_id' ]) * strlen($text) )) / 2;
	$y  = ($CONFIG[ 'height' ] - ImageFontHeight($CONFIG[ 'font_id' ]) ) / 2;
