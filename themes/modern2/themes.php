<?php 
	...
	**************************************************************************/
	...
	function theme_pagelayout () {
		...
		$blocks_top	= theme_menu('T');
		$blocks_lef	= theme_menu('L');
		$blocks_rig	= theme_menu('R');
		$blocks_bot	= theme_menu('B');

		if ($blocks_top == '' && $blocks_lef == '' && $blocks_rig == '' && $blocks_bot == '') {
			$blocks_rig	= theme_menu();
		}
		$num_menus = 0;
		if ($blocks_rig != '') $num_menus += 1;
		if ($blocks_lef != '') $num_menus += 1;
		
		$content_width = $theme_vars[ 'content_width' ];
		$menu_width    = $theme_vars[ 'menu_width' ];
		$page_width    = $content_width + ( $menu_width * $num_menus ); 
			...
			<table border="0" width="<?php echo( $page_width ); ?>" cellspacing="0" cellpadding="0" align="center" style="border: 1px solid #<?php echo( $user_colors[ 'border_color' ] ); ?>;">
				<tr align="left" valign="top">
					<td width="<?php echo( $page_width ); ?>" colspan="2" bgcolor="#<?php echo( $user_colors[ 'header_bg_color' ] ); ?>">
						<div id="header_image"><center><img src="<?php echo( $img_path ); ?>header750x100.jpg" alt="" border="0" /></center></div>
						<div id="header">
						...
						<div id="pagebody">
							<table border="0" width="<?php echo( $page_width ); ?>" cellspacing="0" cellpadding="0" align="center">
								...
								<!-- BLOQUES DE CABECERA -->
								<tr align="left" valign="top">
									<td width="<?php echo( $page_width ); ?>" bgcolor="#<?php echo( $user_colors[ 'menu_bg_color' ] ); ?>" style="border-left: 1px solid #<?php echo( $user_colors[ 'inner_border_color' ] ); ?>" colspan="<?php echo( 1 + $num_menus ); ?>">
										<div id="topbar"><?php echo( $blocks_top ); ?></div>
									</td>
								</tr>
								<?php } ?>
								<tr valign="top">
									<?php if ($blocks_lef != '') { ?>
									<!-- BLOQUES MENU IZQUIERDO -->
										<td width="<?php echo( $menu_width ); ?>" bgcolor="#<?php echo( $user_colors[ 'menu_bg_color' ] ); ?>" style="border-left: 1px solid #<?php echo( $user_colors[ 'inner_border_color' ] ); ?>">
											<div id="sidebar"><?php echo( $blocks_lef ); ?></div>
										</td>
									<?php } ?>
									
									<!-- CONTENIDO -->
									<td width="<?php echo( $content_width ); ?>" bgcolor="#<?php echo( $user_colors[ 'main_bg_color' ] ); ?>">
										<div id="maincontent"><?php page_content(); ?></div>
									</td>
									
									<?php if ($blocks_rig != '') { ?>
									<!-- BLOQUES MENU DERECHO -->
										<td width="<?php echo( $menu_width ); ?>" bgcolor="#<?php echo( $user_colors[ 'menu_bg_color' ] ); ?>" style="border-left: 1px solid #<?php echo( $user_colors[ 'inner_border_color' ] ); ?>">
											<div id="sidebar"><?php echo( $blocks_rig ); ?></div>
										</td>
									<?php } ?>
								...
								<tr align="left" valign="top">
									<td width="<?php echo( $page_width ); ?>" bgcolor="#<?php echo( $user_colors[ 'menu_bg_color' ] ); ?>" style="border-left: 1px solid #<?php echo( $user_colors[ 'inner_border_color' ] ); ?>" colspan="<?php echo( 1 + $num_menus ); ?>">
										<div id="bottonbar"><?php echo( $blocks_bot ); ?></div>
									</td>
								</tr>
								<?php } ?>
							</table>
							<table border="0" width="<?php echo( $page_width ); ?>" cellspacing="0" cellpadding="0" align="center">
								<tr align="left" valign="top">
									<td width="<?php echo( $page_width ); ?>" bgcolor="#<?php echo( $user_colors[ 'footer_bg_color' ] ); ?>" colspan="3">
	...
	function theme_menu ($block_position = '') {
		global $user_colors, $blog_theme, $lang_string, $theme_vars, $logged_in, $sb_info, $blog_config;
		if ($block_position != '') {
			$array = read_blocks($logged_in, $block_position);
			$contenidos = '';
			for($i=0 ; $i<count($array) ; $i+=2) {
				// Si no hay contenido, no pinto nada
				if ( $array[$i+1] != "" ) {
					$contenidos .= "<div class=\"menu_title\">" . $array[$i] . "</div>\n";
					$contenidos .= "<div class=\"menu_body\">\n";
					$contenidos .= $array[$i+1] . "\n";
					$contenidos .= "</div><br />\n";
				}
			}
			return $contenidos;
