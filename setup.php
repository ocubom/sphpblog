...
<?php 
	function page_content() {
		...
		<form action="setup_cgi.php" method="post" name="setup" onSubmit="return validate(this)">
			...
			<input type="text" name="blog_title" value="<?php echo($blog_config[ 'blog_title' ]); ?>" size="40" style="width: <?php global $theme_vars; echo( $theme_vars[ 'max_image_width' ] ); ?>px;" /><br /><br />
			
			<label for="blog_author"><?php echo( $lang_string[ 'blog_author' ] ); ?></label><br />
			<input type="text" name="blog_author" value="<?php echo($blog_config[ 'blog_author' ]); ?>" size="40" style="width: <?php global $theme_vars; echo( $theme_vars[ 'max_image_width' ] ); ?>px;" /><br /><br />
			
			<label for="blog_email"><?php echo( $lang_string[ 'blog_email' ] ); ?></label><br />
			<input type="text" name="blog_email" value="<?php echo($blog_config[ 'blog_email' ]); ?>" size="40" style="width: <?php global $theme_vars; echo( $theme_vars[ 'max_image_width' ] ); ?>px;" /><br /><br />
			
			<label for="blog_footer"><?php echo( $lang_string[ 'blog_footer' ] ); ?></label><br />
			<input type="text" name="blog_footer" value="<?php echo($blog_config[ 'blog_footer' ]); ?>" size="40" style="width: <?php global $theme_vars; echo( $theme_vars[ 'max_image_width' ] ); ?>px;" />
		
			<div class="hr"><hr /></div>
			
			<label for="blog_entry_order"><?php echo( $lang_string[ 'label_entry_order' ] ); ?></label><br />
			<select name="blog_entry_order">
				<option label="<?php echo( $lang_string[ 'select_new_to_old' ] ); ?>" value="new_to_old"<?php if ($blog_config[ 'blog_entry_order' ] == 'new_to_old') { echo ' selected="selected"'; } ?>><?php echo( $lang_string[ 'select_new_to_old' ] ); ?></option>
				<option label="<?php echo( $lang_string[ 'select_old_to_new' ] ); ?>" value="old_to_new"<?php if ($blog_config[ 'blog_entry_order' ] == 'old_to_new') { echo ' selected="selected"'; } ?>><?php echo( $lang_string[ 'select_old_to_new' ] ); ?></option>
			</select><br /><br />
			
			<label for="blog_comment_order"><?php echo( $lang_string[ 'label_comment_order' ] ); ?></label><br /> 
			<select name="blog_comment_order">
				<option label="<?php echo( $lang_string[ 'select_new_to_old' ] ); ?>" value="new_to_old"<?php if ($blog_config[ 'blog_comment_order' ] == 'new_to_old') { echo ' selected="selected"'; } ?>><?php echo( $lang_string[ 'select_new_to_old' ] ); ?></option>
				<option label="<?php echo( $lang_string[ 'select_old_to_new' ] ); ?>" value="old_to_new"<?php if ($blog_config[ 'blog_comment_order' ] == 'old_to_new') { echo ' selected="selected"'; } ?>><?php echo( $lang_string[ 'select_old_to_new' ] ); ?></option>
			</select><br /><br />
			
			<label for="blog_max_entries"><?php echo( $lang_string[ 'blog_max_entries' ] ); ?></label><br />
			<input type="text" name="blog_max_entries" value="<?php echo( $blog_config[ 'blog_max_entries' ] ); ?>" size="6" />
			...
			<input type="text" name="blog_ping_urls" value="<?php echo($blog_config[ 'blog_ping_urls' ]); ?>" size="40" style="width: <?php global $theme_vars; echo( $theme_vars[ 'max_image_width' ] ); ?>px;" /><br />
