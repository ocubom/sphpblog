<?php 
	...
	function page_content() {
		...
		$form_template = new Template(TEMPLATE_DIR.'admin/useradd.tpl');
		...
		if (array_key_exists('submit',$_POST) && ($_POST['submit'] == 'Add User' || $_POST['submit'] == 'Edit User')) {
			...
			$record->setTag('ACTIVE', 				smartstripslashes($_POST['active'])=='on'?				'Y':'N');
			$record->setTag('MODERATE_COMMENTS', 	smartstripslashes($_POST['moderate_comments'])=='on'?	'Y':'N');
			$record->setTag('DELETE_BLOG_ENTRIES', 	smartstripslashes($_POST['delete_blog_entries'])=='on'?	'Y':'N');
			$record->setTag('EDIT_ANY_ENTRY', 		smartstripslashes($_POST['edit_any_entry'])=='on'?		'Y':'N');
			...
			if ($result !== false) { // Success
				...
				$form_template->setTag('{ACTIVE_CHECKED}', 					($record->getTag('ACTIVE')=='Y'?' checked="checked"':''));
				$form_template->setTag('{MODERATE_COMMENTS_CHECKED}', 		($record->getTag('MODERATE_COMMENTS')=='Y'?' checked="checked"':''));
				$form_template->setTag('{DELETE_BLOG_ENTRIES_CHECKED}', 	($record->getTag('DELETE_BLOG_ENTRIES')=='Y'?' checked="checked"':''));
				$form_template->setTag('{EDIT_ANY_ENTRY_CHECKED}', 			($record->getTag('EDIT_ANY_ENTRY')=='Y'?' checked="checked"':''));
			}
